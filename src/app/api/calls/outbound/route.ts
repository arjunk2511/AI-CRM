...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isConfigComplete) {
      // Initialize a local sandbox database call record to let the browser dialer run simulation
      ...
      await dbService.updateConversation(callRecord.id, {
        ...
        summary: "Sandbox phone call simulated locally in dashboard."
      });
      ...
      return NextResponse.json({
        ...
        message: "No live telephony configurations. Running Sandbox Call Emulator."
      });
    }

    // 4. Trigger Real Outbound Dial
    ...
    if (!dialResult.success) {
      return NextResponse.json({ error: dialResult.error || "Dialing trigger failed" }, { status: 500 });
    }
    ...
    console.error("Outbound Call API route error:", error);
    return NextResponse.json({ error: error.message || "Failed to trigger dialer" }, { status: 500 });
  }
}
